...
Vagrant.configure("2") do |config|
  config.vm.define "oel7-puppet" do |vm|
    ...
    #config.vbguest.iso_path = "./virtualbox/VBoxGuestAdditions_7.0.8.iso"
    config.vm.provider "virtualbox" do |vb|
      #vb.vbguest.installer_options = {
      #  allow_kernel_upgrade: true
      #}
      vb.memory = "4096"
    end

    #Update and download puppet package
    #config.vm.provision :shell, path: "./scripts/vboxguest.sh"

    #Copy script
    #config.vm.provision "file", source: "./manifests/mariadb/#mariadb_init.sh", destination: "/tmp/mariadb_init.sh"
    ...
    #   # Comandos de instalación de VirtualBox Guest Additions
    #   sudo yum update -y
